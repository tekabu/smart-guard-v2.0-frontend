...
function initMqttConnection() {
  ...
  try {
    ...
    mqttClient.value.on('connect', () => {
      console.log('Connected to MQTT broker (Faculty Fingerprint)');
    ...
    mqttClient.value.on('close', () => {
      console.log('MQTT connection closed (Faculty Fingerprint)');
...
function disconnectMqtt() {
  if (mqttClient.value) {
    console.log('Disconnecting from MQTT broker (Faculty Fingerprint)');
    isScanning.value = false;
    currentReference.value = null;
    currentPosition.value = null;
    isMqttConnected.value = false;
